...
- 临时站点有有效期，到期后自动下线并从片段移除；也可在面板手动「关闭」。**下线后该子域名即不可访问**。
- 过期记录保留最新 3 条用于追溯，其余删除。

### 一次性配置（在你的 Caddyfile 中）

为泛域名创建 site 块，并在其中 `import` 面板的片段，同时让未匹配的子域名返回空响应：

**HTTPS：（caddy默认无法申请泛域名证书，请自行配置）**
...
**HTTP：** 在站点地址前加 `http://`，Caddy 只监听 80 端口、不申请证书。
推荐这个做法，毕竟临时站点是短期使用的，HTTPS 证书申请与续签都需要额外配置。
...
- `import /etc/ply/ply-temp.caddy`：引入面板生成的临时站点路由。
...
按照官方文档进行安装：https://caddyserver.com.cn/docs/install
...
curl -fsSL https://raw.githubusercontent.com/WenAnrong/ply/main/install.sh | sudo PLY_PORT=18000 bash
```
...
代码更新并推送到仓库后，用下面方式更新服务器上的服务（不会清空数据库与配置）。

```bash
curl -fsSL https://raw.githubusercontent.com/WenAnrong/ply/main/install.sh | sudo bash
```

如果第一次指定了 `PLY_PORT`，之后再次运行安装脚本会复用 `config.ini` 中的端口；无须重复指定，如果指定为其他的则会覆盖原端口。
